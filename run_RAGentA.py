...
def main():
        ...
        try:
            ...
            if args.output_format == "debug":
                debug_output_file = os.path.join(
                    args.output_dir,
                    "debug",
                    f"single_question_debug_{timestamp}_{random_number}.json",
                )
                ...
                output_file = os.path.join(
                    args.output_dir, f"single_question_{timestamp}_{random_number}.json"
                )
    ...
    for i, item in enumerate(questions):
        ...
        try:
            ...
            debug_output_file = os.path.join(
                args.output_dir,
                "debug",
                f"question_{question_id}_debug_{timestamp}_{random_number}.json",
            )
            with open(debug_output_file, "w", encoding="utf-8") as f:
    ...
    if results:
        if args.output_format == "jsonl":
            output_file = os.path.join(
                args.output_dir, f"answers_{timestamp}_{random_number}.jsonl"
            )
            ...
            for result in results:
                ...
                output_file = os.path.join(
                    args.output_dir,
                    f"answer_{question_id}_{timestamp}_{random_number}.json",
                )
                write_result_to_json(result, output_file)
        else:  # debug
            output_file = os.path.join(
                args.output_dir,
                "debug",
                f"all_results_debug_{timestamp}_{random_number}.json",
            )
            with open(output_file, "w", encoding="utf-8") as f:
