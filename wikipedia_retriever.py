...
class WikipediaRetriever:
    def __init__(
        ...
        wiki_path="data/documents/wikipedia",
        use_cached=True,
        max_passages=100000,  # Limit for initial implementation
    ):
        """
        ...
        Args:
            ...
            max_passages: Maximum number of passages to process (for testing/development)
        """
        ...
        if (
            ...
            and os.path.exists(self.passages_path)
        ):
    ...
    def prepare_wikipedia(self):
        ...
        wiki_data = load_dataset(
            "wikipedia", "20220301.en", split=f"train[:{self.max_passages}]", trust_remote_code=True
        )
    ...
    def retrieve(self, query, top_k=20):
        ...
        distances, indices = self.index.search(
            np.array(query_embedding).astype("float32"), min(top_k, len(self.passages))
        )
...
if __name__ == "__main__":
    # Initialize retriever (set use_cached=False for first run)
    retriever = WikipediaRetriever(use_cached=True, max_passages=10000)
    ...
    for i, doc in enumerate(docs):
        print(f"\n{i+1}. {doc[:200]}...")  # Print just the beginning of each doc
