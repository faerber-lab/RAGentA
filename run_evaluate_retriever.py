...
def run_evaluation(retriever, questions, golden_docs, ks=[5, 10, 20, 50, 100]):
    ...
    for question in tqdm(questions, desc="Retrieving documents"):
        ...
        try:
            ...
            normalized_retrieved_ids = [normalize_id(doc_id) for doc_id in raw_retrieved_ids]
            ...
            intersection = set(normalized_retrieved_ids[:max(ks)]).intersection(set(normalized_golden_ids))
            ...
            logger.info(f"  Normalized Retrieved IDs count: {len(normalized_retrieved_ids[:max(ks)])}")
            logger.info(f"  Intersection count: {len(intersection)}")
            
            # Log the first few IDs for comparison
            if raw_golden_ids:
                logger.info(f"  First golden ID: {raw_golden_ids[0]} → {normalized_golden_ids[0]}")
            if raw_retrieved_ids:
                logger.info(f"  First retrieved ID: {raw_retrieved_ids[0]} → {normalized_retrieved_ids[0]}")
    ...
    results["comparison_summary"] = {
        ...
        "average_golden_ids": sum(item["golden_count"] for item in comparison_data) / len(comparison_data) if comparison_data else 0,
        "average_matches": sum(item["intersection_count"] for item in comparison_data) / len(comparison_data) if comparison_data else 0,
    }
...
def main():
    ...
    parser.add_argument("--output_file", type=str, default="retrieval_eval_results.json",
    ...
    # Save detailed comparison data
    comparison_file = os.path.join(os.path.dirname(args.output_file), 
                                  f"comparison_data_{int(time.time())}.json")
