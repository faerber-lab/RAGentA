...
class HybridRetriever:
    ...
    def _get_opensearch_client(self, profile=AWS_PROFILE_NAME, region=AWS_REGION_NAME):
        ...
        host_name = self._get_ssm_value("/opensearch/endpoint", profile=profile, region=region)
    ...
    def _embed_query(self, query):
        ...
        with torch.no_grad():
            inputs = self.tokenizer([query_with_prefix], return_tensors="pt", padding=True, truncation=True)
    ...
    def retrieve(self, query, top_k=None):
        ...
        for res in combined.values():
            res["final_score"] = self.alpha * res["semantic_score"] + (1 - self.alpha) * res["keyword_score"]
        
        # Sort and limit
        ranked_results = sorted(combined.values(), key=lambda x: x["final_score"], reverse=True)[:top_k]
