...
def evaluate_with_datamorgana(rag_system, qa_pairs, output_file=None):
    ...
    for qa_pair in tqdm(qa_pairs, desc="Evaluating QA pairs"):
        ...
        try:
            ...
            print(f"τq: {debug_info['tau_q']:.4f}, Filtered docs: {len(debug_info['filtered_docs'])}")
...
def main():
    parser = argparse.ArgumentParser(description="Run MAIN-RAG with Pinecone and Falcon")
    parser.add_argument(
        "--pinecone_key", required=True,
        help="Pinecone API key"
    )
    parser.add_argument(
        "--falcon_key", required=True,
        help="AI71 API key for Falcon"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--query", 
        help="Single query to answer (if not using --qa_file)"
    )
